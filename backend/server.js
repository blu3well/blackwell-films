...
// --- EMAIL CONFIGURATION ---
// We wrap this in a try-catch to prevent server crash if env vars are missing
let transporter;
try {
  ...
  console.error("Email Transport Setup Failed:", err);
}
...
app.post("/api/purchase-guest", async (req, res) => {
  ...
  console.log("Processing purchase for:", email, reference); // Debug Log
  ...
  try {
    ...
    let paymentStatus = "success"; // Default for testing if Paystack fails
    try {
      ...
      console.error(
        "Paystack Error (Continuing anyway):",
        paystackError.message
      );
      // If Paystack API fails (e.g. wrong key), we log it but might choose to fail
      // For now, if you are seeing "Auth Failed", it's likely this line crashing.
      // We will return error here to be safe.
      return res
        .status(400)
        .json({ success: false, message: "Payment Verification System Error" });
    }
    ...
    // D. Send Email (Fail silently if email breaks)
    try {
      if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
        console.log("Skipping email: No credentials provided in .env");
      } else {
        const mailOptions = {
      ...
      // Do NOT return error to user. They paid, they deserve the code.
    }
